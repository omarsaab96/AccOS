      ...
      {openDocuments.map((doc) => {
        ...
        return (
          <motion.div
            key={doc.id}
            className={`
              inline-flex items-center ltr:pl-[10px] ltr:pr-[30px] rtl:pr-[10px] rtl:pl-[30px] pt-[2px] pb-[6px] text-sm rounded-md mr-1 leading-1 cursor-pointer select-none relative
              ${isActive
          ...
          >
            ...
            {doc.isDirty && (
              <span className="ltr:ml-1.5 rtl:mr-1.5 text-gray-500 dark:text-gray-400 text-xs">●</span>
            )}
            <button
              className="absolute top-[50%] -translate-y-[50%] ltr:right-[10px] rtl:left-[10px] w-[15px] h-[15px] outline-none text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-full"
              onClick={(e) => {
                e.stopPropagation();
                closeDoc(doc.id);
              }}
